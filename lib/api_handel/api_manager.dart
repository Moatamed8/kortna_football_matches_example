...
class SoccerApi {
  ...
  Future<List<InjuPlayer>> getInjured() async {
    var res = await http.get(
        Uri.parse("https://v3.football.api-sports.io/injuries?fixture=686314"),
        headers: headers);
    ...
    if (res.statusCode == 200) {
      ...
      matches =
          injList.map((dynamic item) => InjuPlayer.fromJson(item)).toList();
    }
    return matches;
  }
  ...
  Future<List<SoccerMatch>> getSeason2021matches() async {
    return getSeasonmatches("2024");
  }

  Future<List<SoccerMatch>> getSeason2020matches() async {
    return getSeasonmatches("2023");
  }
